 ...
 * 
 * paths:
 *   /auth/login-two-factor-auth:
 *     post:
 *       security:
 *         - bearerAuth: []
 *       tags:
 *         - Authentication
 *       summary: Second factor authentication login
 *       description: Completes two factor authentication login.
 *       parameters:
 *         - name: token
 *           in: query
...
router
  .route(`${BASE_ROUTE}/login-two-factor-auth`)
  .post(
    Passport.authenticate("jwt", { session: false }),
    controller.completeTwoFactorAuthLogin
  );
